...
R:
CREATE INDEX kontrola_id_straznik_indx
  ON  kontrola(id_straznik);
...
R:
	CREATE VIEW StraznikNagrody AS 
		SELECT imie, nazwisko, nazwa, data_przyznania 
		FROM  straznik s JOIN  przyznane_nagrody pn ON (s.id=pn.straznik_id);

SELECT * FROM StraznikNagrody
